...
该模块实现了多种复杂的迁移学习策略，包括：
- 加权域适应 (weighted_domain_adaptation)
- 伪标签迁移学习 (pseudo_labeling_transfer)
- 对抗域适应迁移学习 (adversarial_domain_adaptation)
"""
...
def weighted_domain_adaptation(
    ...
    # 训练新模型，使用源域数据作为增强训练集
    print(f"Training with weighted domain adaptation (source:{source_weight}, target:{target_weight}) on {device.upper()}...", flush=True)
...
def pseudo_labeling_transfer(
    ...
    # 为目标域无标签数据生成伪标签
    unlabeled_features = target_unlabeled_data[x_cols].values
    ...
    # 结合目标域的真实数据和伪标签数据
    # 注意：这里假设 target_unlabeled_data 包含真实标签。如果它是完全无标签的，
    # 应该是 pd.concat([target_unlabeled_data, pseudo_labeled_data])
